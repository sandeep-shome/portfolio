  ...
  return (
    <div className="py-10">
      ...
      <div className="flex lg:flex-row justify-between items-center flex-col mt-20">
        <Heading className="font-black mb-2 pb-1"> {article.title}</Heading>
        <div className="flex space-x-2 md:mb-1 mt-2 md:mt-0">
          {article.stack?.map((stack: string) => (
            <span
              key={stack}
      ...
      <div>
        <Paragraph className="max-w-xl mt-4">{article.description}</Paragraph>
      </div>
      <div className="prose prose-sm md:prose-base max-w-none text-neutral-600">
        {article?.content}
      </div>
      {article.linkPlaceholder && (
        <a
          href={article.href}
          target="__blank"
